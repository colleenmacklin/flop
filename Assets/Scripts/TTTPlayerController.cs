...
public class TTTPlayerController : MonoBehaviour
{
    ...
    void Start()
    {
        penDraw = myPen.GetComponent<Draw>();
    ...
    public IEnumerator DrawXAtPosition(Vector3 position)
    {
        ...
        myPen.transform.position = startPos1;
    ...
    public IEnumerator RestPen()
    {
        ...
        {
            myPen.transform.position = Vector3.MoveTowards(myPen.transform.position, target, 0.1f);
    ...
    public IEnumerator ChasePlayer()
    {
        ...
        while (true)
        {
            penDraw.isActive = true;
            penDraw.forceDraw = true;
            myPen.transform.position = Vector3.MoveTowards(myPen.transform.position, playerPen.transform.position, 0.1f);
            yield return new WaitForSeconds(0.01f);
        }
    }
    ...
    {
        while (Vector3.Distance(myPen.transform.position, target) > 0.01f)
        {
            myPen.transform.position = Vector3.MoveTowards(myPen.transform.position, target, 0.1f);
            yield return new WaitForSeconds(0.01f);
        }
    ...
    {
        while (Vector3.Distance(myPen.transform.position, target) > 0.2f)
        {
            float distanceFactor = Vector3.Distance(myPen.transform.position, target) / Vector3.Distance(myPen.transform.position, playerPen.transform.position);
            ...
            myPen.transform.position = Vector3.MoveTowards(myPen.transform.position + wiggleOffset, target, 0.1f);
            yield return new WaitForSeconds(0.01f);
        }
    }
}
